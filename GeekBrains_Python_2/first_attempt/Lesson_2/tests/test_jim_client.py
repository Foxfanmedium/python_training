...
import network.jim.client.py

from datetime import datetime

from jim.client import JSONRequest, JSONRequest
from python_training.GeekBrains_Python_2.Lesson_2.network.jim import settings
...
def request(action, time, to_client, from_client, body):
    return JSONRequest(action,body, to = to_client, frm=from_client)
...
def test remove_header(request, from_client):
